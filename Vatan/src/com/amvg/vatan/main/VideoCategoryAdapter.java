...
public class VideoCategoryAdapter extends ArrayAdapter<String> {
	...
	public VideoCategoryAdapter(Context context, String[] values, String[][] realValues, String isVideo, Context contextDialog) {
...
//		}
		else 
		{
			...
			if (isVideo.equals("1")) //video ise.
			{
				Bitmap bitmap = BitmapFactory.decodeFile("/sdcard/Vatan/videocategory/"+realValues[position-bannerK][4]);
			    jpgView.setImageBitmap(bitmap);
			}
			else
			{
				((ImageView)convertView.findViewById(R.id.playIcon)).setVisibility(View.INVISIBLE);
				Bitmap bitmap = BitmapFactory.decodeFile("/sdcard/Vatan/gallerycategory/"+realValues[position-bannerK][5]);
			    jpgView.setImageBitmap(bitmap);
			}
