...
public class JSon extends AsyncTask<String, String, String>{
	...
	protected String doInBackground(String... parseParam) {
		...
		try {
			...
			String[] values = new String[parseParam.length-1];			
			for (int i = 0; i < values.length; i++) {
					values[i]=jObject.getString(parseParam[i+1].toString());
			}*/
        	 ...
        	 AlertDialog.Builder builder = new AlertDialog.Builder(context);
      		...
      		builder.setPositiveButton(R.string.update_simdi, new DialogInterface.OnClickListener() {
      	           public void onClick(DialogInterface dialog, int id) {
      	        	 context.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("http://play.google.com/store/apps/details?id="+SplashScreen.bundleId)));
